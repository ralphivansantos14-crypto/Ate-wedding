...
(function initRSVP() {
  ...
  submitBtn.addEventListener('click', () => {
    ...
    formData.append('bot-field', ''); // honeypot — leave blank
...
(function initMusic() {
  ...
  // Called once on the very first user gesture
  function autoStart() {
  ...
  // Listen on document for ANY first interaction — touch, scroll, or key
  ['touchstart', 'touchend', 'pointerdown', 'keydown', 'scroll'].forEach(evt => {
    document.addEventListener(evt, autoStart, { once: true, passive: true, capture: true });
  });

  // Manual toggle button
  btn.addEventListener('click', (e) => {
    // If autoStart hasn't fired yet, mark it done so it doesn't double-play
    autoStarted = true;
